...
/*DEBUG*/ #include <iostream>
     ...
     * Eigen namespace. */
    namespace Eigen {
  enum { RowPermutation, ColPermutation, RowAndColPermutation };

  template <typename MatrixType>
  struct ei_compute_lvalue_bit {
    enum {
      ret = (MatrixType::Flags & (DirectAccessBit | LvalueBit)) ? LvalueBit : 0
    };
  };
  ...
  struct traits<SubMatrix<MatrixType, PermutationType, IsSub> >
    ...
    enum {
      RowsAtCompileTime = (PermutationType == ColPermutation)
                              ? (MatrixType::RowsAtCompileTime)
                              : Dynamic,
      ColsAtCompileTime = (PermutationType == RowPermutation)
                              ? (MatrixType::ColsAtCompileTime)
                              : Dynamic,
      MaxRowsAtCompileTime =
          (IsSub ? MatrixType::MaxRowsAtCompileTime : Dynamic),
      MaxColsAtCompileTime =
          (IsSub ? MatrixType::MaxColsAtCompileTime : Dynamic),
      VectorAtCompileTime =
          (RowsAtCompileTime == 1) || (ColsAtCompileTime == 1),
      Flags = ((MatrixType::Flags & HereditaryBits) |
               ei_compute_lvalue_bit<MatrixType>::ret |
  ...
  struct evaluator<SubMatrix<MatrixType, PermutationType, IsSub> >
    ...
    typedef typename SubMatrix<MatrixType, PermutationType, IsSub>::MemoryBase
        MemoryBase;
    enum {
    ...
    evaluator(const XprType& xpr) : m_argImpl(xpr) {}
    ...
    inline Scalar& coeffRef(Index row) { return m_argImpl.coeffRef(row); }
  ...
  struct ei_submatrix_index_helper<MatrixType, RowPermutation,
  ...
  template <typename MatrixType, int rowsAtCompileTime, int colsAtCompileTime>
  struct ei_submatrix_index_helper<MatrixType, ColPermutation,
                                   rowsAtCompileTime, colsAtCompileTime> {
    typedef typename MatrixType::Index Index;
  ...
  struct ei_submatrix_index_helper<MatrixType, RowAndColPermutation, 1,
                                   colsAtCompileTime> {
    typedef typename MatrixType::Index Index;
    static Index index(Index /*row*/, Index col) { return col; }
  };
  ...
  // TODO: Void containter is not useful anymore, neither is
  // ei_choose_container. Remove them, and propagate the changes in submatrix.
  template <typename MatrixType>
  class VoidContainer {
   protected:
  ...
  class RowSelectionImpl {
   public:
    ...
    RowSelectionImpl(const RowSelectionImpl& clone)
        : m_contain(clone.m_contain),
          owned_rowIndices(clone.owned_rowIndices),
          rowIndices(clone.rowIndicesOwned() ? owned_rowIndices
                                             : clone.rowIndices) {}
    ...
    void setRowRange(Index start, Index end) {
      eigen_assert(0 <= start);
      eigen_assert(inMRange(std::max<long>(0, end - 1)));
   ...
   private:
    ...
    bool inMRange(Index i) const {
      return 0 <= i && i < m_contain.m_matrix.rows();
    }
  ...
  class ColSelectionImpl {
   public:
    ...
    ColSelectionImpl(const ColSelectionImpl& clone)
        : m_contain(clone.m_contain),
          owned_colIndices(clone.owned_colIndices),
          colIndices(clone.colIndicesOwned() ? owned_colIndices
                                             : clone.colIndices) {}
   ...
   private:
    ...
    bool inMRange(Index i) const {
      return 0 <= i && i < m_contain.m_matrix.cols();
    }
  ...
  template <typename MatrixType, int PermutationType = RowAndColPermutation,
            bool IsSub = true>
  class SubMatrix
   ...
   public:
    ...
    typedef typename ei_choose_container_impl<MatrixType, PermutationType>::type
        MemoryBase;
    typedef
        typename ei_choose_row_impl<MatrixType, PermutationType, IsSub>::type
            RowBase;
    typedef
        typename ei_choose_col_impl<MatrixType, PermutationType, IsSub>::type
            ColBase;
    ...
    inline SubMatrix(MatrixType& matrix)
        : MemoryBase(matrix), RowBase(matrix, false), ColBase(matrix, false) {}
    ...
    inline SubMatrix(MatrixType& matrix, bool defaultPermutationRows,
    ...
    inline SubMatrix(MatrixType& matrix, const Indices& indicesRows,
                     bool defaultPermutationCols)
        : MemoryBase(matrix),
    ...
    inline SubMatrix(MatrixType& matrix, Indices* indicesRows,
                     Indices* indicesCols)
        : MemoryBase(matrix),
          RowBase(matrix, indicesRows),
          ColBase(matrix, indicesCols) {}
    ...
    inline Scalar& coeffRef(Index row, Index col) const {
      return MemoryBase::m_matrix.const_cast_derived().coeffRef(
          this->rowIndex(row), this->colIndex(col));
    }

    inline Scalar& coeffRef(Index index) const {
      return MemoryBase::m_matrix.const_cast_derived().coeffRef(
          ei_submatrix_index_helper<MatrixType, PermutationType>::index(
              this->rowIndex(index), this->colIndex(index)));
    }

    inline const CoeffReturnType coeff(Index row, Index col) const {
      return MemoryBase::m_matrix.coeff(this->rowIndex(row),
                                        this->colIndex(col));
    }
  ...
  template <typename MatrixType, bool IsSub = true>
  class SubCol : protected ColContainer<MatrixType>,
                 public SubMatrix<typename ColContainer<MatrixType>::NestedType,
  ...
  namespace internal {
  template <typename MatrixType1, typename MatrixType2>
  struct traits<StackMatrix<MatrixType1, MatrixType2> > : traits<MatrixType1> {
    ...
    enum {
      ...
      Flags = (_MatrixTypeNested::Flags & HereditaryBits) |
              ei_compute_lvalue_bit<_MatrixTypeNested>::ret,
      CoeffReadCost =
          evaluator<_MatrixTypeNested>::CoeffReadCost  // Todo : check that
    };
  };
  }  // namespace internal
  ...
  class StackMatrix
   ...
   public:
    ...
    inline StackMatrix(Base1& m1, Base2& m2) : m1(m1), m2(m2) {
  ...
  struct evaluator<StackMatrix<MatrixType1, MatrixType2> >
    ...
    enum {
      ...
      Flags = ((MatrixType1::Flags & HereditaryBits) |
               ei_compute_lvalue_bit<MatrixType1>::ret) &
              ~LinearAccessBit
    };

    evaluator(const XprType& xpr) : m1(xpr.m1), m2(xpr.m2) {}

    inline Scalar& coeffRef(Index row, Index col) {
      const Index r1 = m1.rows();
      if (row < r1)
        return m1.coeffRef(row, col);
      else
        return m2.coeffRef(row - r1, col);
    }
    inline Scalar& coeffRef(Index index) {
      const Index r1 = m1.rows();
      if (index < r1)
        return m1.coeffRef(index);
      else
        return m2.coeffRef(index - r1);
    }
    inline const CoeffReturnType coeff(Index row, Index col) const {
      const Index r1 = m1.rows();
      if (row < r1)
